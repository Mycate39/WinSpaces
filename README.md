...
> **Statut :** v0.1 — projet en cours de développement. Les bases (COM interne, hotkeys, momentum, icône) sont fonctionnelles ; le parsing HID Précision Touchpad est *best-effort*.
...
- **Windows 10 1809+** ou **Windows 11** (API interne testée jusqu'à Windows 11 23H2).
...
src/WinSpaces/
  ...
  Native/
    ...
    NativeMethods.cs                   ← DllImport user32/kernel32
...
- **COM interne** (`IVirtualDesktopManagerInternal`) via `IServiceProvider10` + `CLSID_ImmersiveShell` — exactement comme le fait le Shell Windows lui-même. API officielle `IVirtualDesktopManager` utilisée en fallback et pour le déplacement de fenêtres par GUID.
- **Hooks Win32** : `SetWinEventHook` (foreground) + `SetWindowsHookEx` (molette) — aucun hook n'injecte du code, ils ne font que surveiller.
- **Raw Input** : pour le Précision Touchpad, le parsing des rapports HID est *best-effort* selon la spé Microsoft « Windows Precision Touchpad » ; les seuils peuvent nécessiter un ajustement selon le constructeur.

## Limitations connues

- **Windows 11 24H2+** a modifié les interfaces COM internes (nouveaux GUIDs/méthodes). Le support sera ajouté (référence : VirtualDesktop11-24H2.cs de MScholtes).
- Le parsing HID Précision Touchpad est *best-effort* : la structure des rapports varie légèrement selon les fabricants. Le mode momentum (2 doigts / molette) fonctionne sur tous les appareils en complément.
